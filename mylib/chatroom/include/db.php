...
class Db extends Query{
    ...
    /**
     * 返回 adn | or的子查询语句
     * @param string $find  被查找 的值 
     * @param string $field  被查找 的字段名称 
     * @param string $tip    and | or 标识  
     * @return string
     */
    public function childsql($find,$field,$tip){
        $find = explode(',', $find);
        $childsql = '';
        for($i = 0 ;$i < count($find); $i ++){
            $childsql .= ' '.$tip.' ' . $field . "= '$find[$i]'";
        }
        
        $childsql = ($tip == 'or') ? substr($childsql, 3) :  substr($childsql, 4);
        return $childsql;
    }
