<?php
define('EDBUG_ANALYSIS_VERSION', '1.0');
class EdbugAnalysisException extends \Exception {
}
// 在发送的数据格式有误时，SDK会抛出此异常，用户应当捕获并处理。
class EdbugAnalysisIllegalDataException extends EdbugAnalysisException {
}
// 在因为网络或者不可预知的问题导致数据无法发送时，SDK会抛出此异常，用户应当捕获并处理。
class EdbugAnalysisNetworkException extends EdbugAnalysisException {
}
// 当且仅当DEBUG模式中，任何网络错误、数据异常等都会抛出此异常，用户可不捕获，用于测试SDK接入正确性
class EdbugAnalysisDebugException extends \Exception {
}
class EdbugAnalysis {
    private $_consumer;
    private $_super_properties;
    /**
     * 初始化一个 EdbugAnalysis 的实例用于数据发送。
     *
    ...
    public function __construct($consumer) {
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) == "WIN") {
            throw new EdbugAnalysisException("Sensors Analytics PHP SDK dons't not support Windows");
        }
    ...
    private function _normalize_data($data) {
        ...
        if (!isset($data['distinct_id']) or strlen($data['distinct_id']) == 0) {
            throw new EdbugAnalysisIllegalDataException("property [distinct_id] must not be empty");
        }
        if (strlen($data['distinct_id']) > 255) {
            throw new EdbugAnalysisIllegalDataException("the max length of [distinct_id] is 255");
        }
        ...
        if ($ts_num < 10 || $ts_num > 13) {
            throw new EdbugAnalysisIllegalDataException("property [time] must be a timestamp in microseconds");
        }
        if ($ts_num == 10) {
            $ts *= 1000;
        }
        ...
        if (isset($data['event']) && !preg_match($name_pattern, $data['event'])) {
            throw new EdbugAnalysisIllegalDataException("event name must be a valid variable name. [name='${data['event']}']");
        }
        // 检查 properties
        if (isset($data['properties']) && is_array($data['properties'])) {
            foreach ($data['properties'] as $key => $value) {
                if (!is_string($key)) {
                    throw new EdbugAnalysisIllegalDataException("property key must be a str. [key=$key]");
                }
                if (strlen($data['distinct_id']) > 255) {
                    throw new EdbugAnalysisIllegalDataException("the max length of property key is 256. [key=$key]");
                }
                if (!preg_match($name_pattern, $key)) {
                    throw new EdbugAnalysisIllegalDataException("property key must be a valid variable name. [key='$key']]");
                }
                // 只支持简单类型或数组或DateTime类
                if (!is_scalar($value) && !is_array($value) && !$value instanceof DateTime) {
                    throw new EdbugAnalysisIllegalDataException("property value must be a str/int/float/datetime/list. [key='$key']");
                }
                ...
                if (is_string($value) && strlen($data['distinct_id']) > 8191) {
                    throw new EdbugAnalysisIllegalDataException("the max length of property value is 8191. [key=$key]");
                }
                // 如果是数组，只支持 Value 是字符串格式的简单非关联数组
                if (is_array($value)) {
                    if (array_values($value) !== $value) {
                        throw new EdbugAnalysisIllegalDataException("[list] property must not be associative. [key='$key']");
                    }
                    foreach ($value as $lvalue) {
                        if (!is_string($lvalue)) {
                            throw new EdbugAnalysisIllegalDataException("[list] property's value must be a str. [value='$lvalue']");
                        }
                    }
                }
            }
            ...
            throw new EdbugAnalysisIllegalDataException("property must be an array.");
        }
        return $data;
    }
    /**
    ...
    public function track($distinct_id, $is_login_id, $event_name, $properties = array()) {
        if (!is_string($event_name)) {
            throw new EdbugAnalysisIllegalDataException("event name must be a str.");
        }
        if (!is_bool($is_login_id)) {
            throw new EdbugAnalysisIllegalDataException("is_login_id must be a bool.");
        }
        if ($properties) {
     ...
     * @param array $properties 事件的属性。
     * @return bool
     * @throws EdbugAnalysisIllegalDataException
     */
    public function track_signup($distinct_id, $original_id, $properties = array()) {
        ...
        if (!$original_id or strlen($original_id) == 0) {
            throw new EdbugAnalysisIllegalDataException("property [original_id] must not be empty");
        }
        if (strlen($original_id) > 255) {
            throw new EdbugAnalysisIllegalDataException("the max length of [original_id] is 255");
        }
...
class DebugConsumer extends AbstractConsumer {
     ...
     * @throws EdbugAnalysisDebugException
     */
    public function __construct($url_prefix, $write_data = True, $request_timeout = 1000) {
        $parsed_url = parse_url($url_prefix);
        if ($parsed_url === false) {
            throw new EdbugAnalysisDebugException("Invalid server url of Sensors Analytics.");
        }
    ...
    public function send($msg) {
        ...
        if ($response['ret_code'] >= 300) {
            throw new EdbugAnalysisDebugException("Unexpected response from EdbugAnalysis.");
        }
    }
    /**
     * 发送数据包给远程服务器。
    ...
    protected function _do_request($data) {
        ...
        if (!$http_response_header) {
            throw new EdbugAnalysisDebugException(
                   "Failed to connect to EdbugAnalysis. [error='" + curl_error($ch) + "']");
        }
